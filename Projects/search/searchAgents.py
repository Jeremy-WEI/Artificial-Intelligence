...
class CornersProblem(search.SearchProblem):
	...
	def __init__(self, startingGameState):
		...
		"*** YOUR CODE HERE ***"
		self.startState = (False, False, False, False, self.startingPosition[0], self.startingPosition[1])
		for i in range(4):
			if self.startingPosition == self.corners[i]:
				self.startState = self.startState[0:i] + (True,) + self.startState[i + 1:]
				break
	...
	def getSuccessors(self, state):
		...
		for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
			...
			nextx, nexty = int(x + dx), int(y + dy)
			if not self.walls[nextx][nexty]:
				nextState = state[0:4] + (nextx, nexty)
				for i in range(4):
					if (nextx, nexty) == self.corners[i]:
						nextState = nextState[0:i] + (True,) + nextState[i + 1:]
						break
...
def closestCorner(state, corners):
	...
	for corner in corners:
		ind = corners.index(corner)
		dist = util.manhattanDistance((x, y), corner)
		if state[ind] == False and dist < closest:
			closest = dist
			i = ind
	return closest, i
...
def cornersHeuristic(state, problem):
	...
	x, y = state[4], state[5]
	numVisited = len(filter(lambda x: x == True, state[0:4]))
	if numVisited == 4:
		return 0
	closest, i = closestCorner(state, corners)
	smallEdge = min(top, right) - 1
	if numVisited < 1:
		closest += 3 * smallEdge
	elif numVisited < 2:
		closest += 2 * smallEdge
